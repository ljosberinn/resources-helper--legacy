...
function appendFiles ($files) {
    foreach ($files as $link => $subInfo) {
        ...
        <?php } elseif ($subInfo["type"] === "css") { ?>
            <link rel="stylesheet" href="<?= $link ?><?= $lastModified ?>" /><?php
        }
    }
}
...
function validateRegistration ($post) {
    if (isset($_POST["registration-mail"]) && !empty($_POST["registration-mail"]) && isset($_POST["registration-pw-1"]) && !empty($_POST["registration-pw-1"]) && isset($_POST["registration-pw-2"]) && !empty($_POST["registration-pw-2"]) && isset($_POST["registration-language"]) && !empty($_POST["registration-language"]) && isset($_POST["registration-api-key"])) {
