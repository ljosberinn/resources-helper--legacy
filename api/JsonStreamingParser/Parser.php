...
class Parser
{
    ...
    public function parse($conn, $userId)
    {
        $this->lineNumber = 1;
        $this->charNumber = 1;
        $eof = false;
        ...
        while (!feof($this->stream) && !$eof) {
            ...
            for ($i = 0; $i < $byteLen; $i++) {
                ...
                $this->consumeChar($line[$i], $conn, $userId);
                $this->charNumber++;
     ...
     * @throws ParsingError
     */
    private function consumeChar($c, $conn, $userId)
    {
        ...
        case self::STATE_IN_NUMBER:
            if (ctype_digit($c)) {
                ...
                $this->consumeChar($c, $conn, $userId);
            }
            break;
