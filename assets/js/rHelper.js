const rHelper = {
  "init": {
    ...
    core() {
      ...
      let unitsFns = [
        "SET_UnitProfitObj",
        "INSRT_unitsCraftingPrice",
        "INSRT_unitsMarketPrice",
        "INSRT_unitsPricePerStrength",
      ];

      $.each(rHelper.data.units, (index, unit) => {
        // warehouses
      ...
      let buildingFns = [
        "INSRT_buildingName",
        "SET_buildingBackgroundColor",
        "INSRT_buildingData",
        "INSRT_buildingToLevel10",
        "EVNT_buildingChange",
      ];
  ...
  "methods": {
    ...
    SET_recyclingPlantLevel() {
      ...
      $.each(rHelper.data.loot, (lootId) => {
        ...
        let fns = [
          ...
          "INSRT_recyclingOutputWorth",
          "INSRT_recyclingInputWorth",
        ];
    ...
    SET_hqLevel() {
      ...
      if (headquarter.user) {
        ...
        let fns = [
          "INSRT_headquarterContentRequiredAmount",
          "INSRT_headquarterMissing",
          "INSRT_headquarterRemainingCost",
        ];
    ...
    SET_mapHQHandler(map, hqObj) {
      ...
      let hqCircle = rHelper.methods.SET_mapHqCircle(map, center, radius, hqObj.relation);
      let hqSize = hqLevel * 12.5;
      let hqIcon = new google.maps.Marker({
        map: map,
        position: center,
    ...
    EVNT_warehouseInput(id, type) {
      ...
      $("#warehouse-" + type + "-stock-current-" + id).on("input", function () {
        ...
        let fns = [
          "INSRT_warehouseWorth",
          "INSRT_warehouseFillStatus",
          "INSRT_warehouseRemainingTimeToFull",
        ];
    ...
    INSRT_materialData(materialId, api) {
      ...
      let fns = [
        "INSRT_materialRate",
        "INSRT_materialAmountOfMines",
        "INSRT_materialNewMinePrice",
        "INSRT_materialRateWorth",
        "INSRT_materialNewMinePerfectIncome",
      ];
    ...
    INSRT_warehouseData(index, type) {
      ...
      let fns = [
        "INSRT_warehouseFillAmount",
        "INSRT_warehouseLevel",
        "INSRT_warehouseFillStatus",
        "INSRT_warehouseCapacity",
        "INSRT_warehouseWorth",
        "EVNT_warehouseInput",
    ...
    INSRT_gaugeGraph(type) {
      ...
      if (type == "buildings") {
        let chart = Highcharts.chart('graph-buildings', Highcharts.merge(rHelper.graphs.gaugeOptions, {
          yAxis: {
            min: 0,
        ...
        let chart = Highcharts.chart('graph-headquarter', Highcharts.merge(rHelper.graphs.gaugeOptions, {
          yAxis: {
            min: 0,
    ...
    INSRT_diamondDependenciesHelper(dependantObj, price, requiredAmount, factoryId, warehouseIconSpan) {
      ...
      let imgSpan = $(crEl("span")).addClass(dependantObj.icon).attr("title", price.toLocaleString("en-US"));
    ...
    INSRT_flowDistributionSingle(id, type) {
      ...
      let distributionTarget = $("#flow-" + type + "-distribution-" + id);
    ...
    INSRT_qualityComparator(type, quality) {
      ...
      $.each(rHelper.data.material, (i, material) => {
        ...
        let currentVisibility = qualityTarget.css("opacity");
        ...
        if (i == type || thisResourcesRequiredAmount > thisMaxRate) {
          ...
          let result = (thisResourcesRequiredAmount / thisMaxRate * 100).toFixed(2) + "%";
    ...
    CALC_factoryTurnoverPerUpgrade(factoryId) {
      ...
      let value = 0;
      let thisLevelWorkload = rHelper.methods.CALC_factoryMinWorkload(factoryId);
