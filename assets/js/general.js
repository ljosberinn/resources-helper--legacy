loadingAnimToggler = (mode, text) => {
  ...
  let rndAnimation =
    possibleAnimationClasses[
      Math.floor(Math.random() * possibleAnimationClasses.length)
    ];
  ...
  $.each([pw1El, pw2El], (i, obj) => {
    if (passwordRegex.test(obj.val()) === false) {
      swal(
        ...
        'minimum of 4 characters including one number\n(e.g. "pas1").',
        "error"
      );
      event.preventDefault();
    }
...
$("#password-change-form").submit(event => {
  ...
  if (passwordRegex.test(pwOldEl.val()) === false) {
    swal(
      ...
      'Your old password does not fit the required pattern: minimum of 4 characters including one number\n(e.g. "pas1").',
      "error"
    );
    event.preventDefault();
  }
...
$("#api-submit").on("click", function(e) {
  ...
  if (!key || key.length != 45) {
    swal(
      "API Error",
      `Key missing or too short/long (${key.length} instead of 45 characters)`,
      "error"
    );
    return;
  }
